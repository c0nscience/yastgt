...
func Test_Apply(t *testing.T) {
	t.Run("should apply the given transform to all points of a path", func(t *testing.T) {
		subj := transform.Apply([]*mat.Dense{
		...
		assert.Equal(t, svg.Point{X: 7, Y: 8}, subj[0].Points[0])
		assert.Equal(t, svg.Point{X: 12, Y: 9}, subj[0].Points[1])
	...
	t.Run("should apply multiple transforms", func(t *testing.T) {
		...
		assert.Equal(t, svg.Point{X: 5.232050807568878, Y: 8.598076211353316}, subj[0].Points[0])
		assert.Equal(t, svg.Point{X: 9.062177826491071, Y: 11.964101615137753}, subj[0].Points[1])
		assert.Equal(t, svg.CubicPoint{
	...
	t.Run("should return same path if not transformation is provided", func(t *testing.T) {
		...
		assert.Equal(t, svg.Point{X: 2, Y: 3}, subj[0].Points[0])
		assert.Equal(t, svg.Point{X: 7, Y: 4}, subj[0].Points[1])
		assert.Equal(t, svg.CubicPoint{
			CP: svg.Point{X: 2, Y: 11},
			P1: svg.Point{X: 14, Y: 18},
			P2: svg.Point{X: 21, Y: 26},
