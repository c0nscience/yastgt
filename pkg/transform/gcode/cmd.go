...
func G5(p *svg.CubicPoint, cp *svg.Point, f float64) Cmd {
	x := floatToString(p.CP.X)
	y := floatToString(p.CP.Y)
	p1 := p.P1.RelativeTo(cp)
	p2 := p.P2.RelativeTo(p.CurrPt())
	p1x := floatToString(p1.X)
	p1y := floatToString(p1.Y)
	p2x := floatToString(p2.X)
	p2y := floatToString(p2.Y)
	return Cmd(fmt.Sprintf("G5 F%.2f I%s J%s P%s Q%s X%s Y%s", f, p1x, p1y, p2x, p2y, x, y))
}

func G2(c *svg.CirclePoint) Cmd {
	return Cmd(fmt.Sprintf("G2 I%s J%s", floatToString(-c.R), floatToString(0)))
}
